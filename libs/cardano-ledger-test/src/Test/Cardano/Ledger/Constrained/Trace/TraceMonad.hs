...
getTarget :: Target era a -> TraceM era a
getTarget tar = do
  env <- getEnv
...
refputs :: Proof era -> TxBody era -> Set (TxIn (EraCrypto era))
refputs (Shelley _) _ = Set.empty
refputs (Allegra _) _ = Set.empty
refputs (Mary _) _ = Set.empty
refputs (Alonzo _) _ = Set.empty
refputs (Babbage _) txb = txb ^. referenceInputsTxBodyL
refputs (Conway _) txb = txb ^. referenceInputsTxBodyL
...
compileTraceWithSubst :: OrderInfo -> Subst era -> [Pred era] -> TraceM era (DependGraph era)
compileTraceWithSubst info subst0 cs0 = do
  ...
  graph <- liftTyped $ do
    ...
    mkDependGraph (length orderedNames) [] Set.empty orderedNames [] (Prelude.filter notBefore instanPreds)
  pure graph
...
universeTrace proof subst = toolChainTrace proof standardOrderInfo (universePreds proof) subst
...
ledgerStateTrace proof subst = toolChainTrace proof standardOrderInfo (ledgerStatePreds proof) subst
