...
import Test.Cardano.Ledger.Constrained.Trace.TraceMonad (TraceM, fromSetTerm, getTerm, liftGen, refputs, reqSig)
...
completeTxBody proof txBody = do
  ...
  let loop _ _ fee _ | fee >= maxFeeEstimate = trace ("LOOP DISCARD") $ discard
      loop count txx fee hash = do
        ...
        if (fee == newfee) && (hash == hash2)
          then pure (completedtx, txb, hash2)
          else
            if count > (0 :: Int)
              then loop (count - 1) completedtx newfee hash2
              else error ("\nSTOP loop count went negative\n" ++ show (pcTx proof txx) ++ " " ++ show fee ++ " " ++ show hash)
  (finaltx, _finalbody, _finalhash) <- loop 5 tx initialfee hash1
  pure finaltx
