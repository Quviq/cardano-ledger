...
solveMap v1@(V _ r@(MapR dom rng) _) predicate = explain msg $ case predicate of
  ...
  (MapMember exprK exprV (Right (Var v2@(V _ (MapR dom2 rng2) _)))) | Name v1 == Name v2 -> do
    ...
    mapSpec
      ...
      (PairSpec dom rng VarOnRight (Map.singleton k v))
      (RngRel (relSuperset rng (Set.singleton v)))
  (List (Var v2@(V _ _ _)) xs)
    | Just Refl <- sameName v1 v2
    , null xs ->
        mapSpec (SzExact 0) RelAny PairAny RngAny
  (List (Var v2@(V _ _ _)) xs@(x : _))
    | Just Refl <- sameName v1 v2
    , Right (With _) <- runTyped $ hasOrd rng rng
    , PairR dl rl <- termRep x -> do
        Refl <- sameRep dom dl
        Refl <- sameRep rng rl
        ...
        mapSpec
          (SzExact (length ys))
          (relEqual dl (makeFromList @(Set dom) ds))
          (PairSpec dom rng VarOnRight (Map.fromList ys))
          (RngRel (relEqual rl (makeFromList @(Set rng) rs)))
